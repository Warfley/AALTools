...
begin
  ...
  try
    ...
    TextColor(White);
    Write(UTF8Decode('.aalproj Dateien verknüpfen? (y/n)'));
    ReadLn(inp);
    if LowerCase(inp) = 'y' then
    begin
      a := TFileAssociation.Create(nil);
      try
        a.ApplicationDescription := 'AET: AAL IDE';
        a.Extension := '.aalproj';
        a.ExtensionName := 'AET Project';
        a.Action := '"' + selDir + 'AALIDE.exe" "%1"';
        a.ActionName := 'Open';
        a.Execute;
      finally
        a.Free;
    ...
    if LowerCase(inp) = 'y' then
    begin
      CreateLink(selDir + 'AALIDE.exe', GetEnvironmentVariable('HOMEDRIVE') +
        GetEnvironmentVariable('HOMEPATH') +
        PathDelim + 'AppData\Roaming\Microsoft\Windows\Start Menu\Programs\AET.lnk', 'AET AAL IDE');
    end;
    TextColor(Yellow);
    ...
    begin
      ...
      try
        p.Executable := selDir + 'AALIDE.exe';
        p.Execute;
      finally
        p.Free;
      end;
